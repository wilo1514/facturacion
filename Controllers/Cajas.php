...
class Cajas extends Controller
{
    ...
    public function listar()
    {
        ...
        $data = $this->model->getVentas($id_usuario);
    ...
    public function transferencia()
    {
        ...
        $id_usuario = $_SESSION['id_usuario'];
        $subtotal = $this->model->calcularVentaNorm($id_usuario);
        $subtotaliess = $this->model->calcularVentaIess($id_usuario);
        $subtotaltarjeta = $this->model->calcularVentaTarjeta($id_usuario);
        $subtotalsin = $this->model->calcularVentaSinTarjeta($id_usuario);
        $subtotalTotal = $subtotal['total'] ?? 0;
        $subtotalIessTotal = $subtotaliess['total'] ?? 0;
        $subtotalTarjetaTotal = $subtotaltarjeta['total'] ?? 0;
        $subtotalSinTotal = $subtotalsin['total'] ?? 0;
        $total = $this->model->calcularVenta($id_usuario);
        $final = $total['total'] + $apertura;
        $rep = $this->model->verificarReporte($id_usuario);
        if ($rep['numero'] == 0) {
            $comp = $this->model->registrarReporte($id_usuario, $apertura, $final, $subtotalTotal, $subtotalIessTotal, $subtotalTarjetaTotal, $subtotalSinTotal, $total['total'], $observacion);
        } else {
            $id_reporte = $this->model->id_reporte($id_usuario);
            $comp = $this->model->setDatos($apertura, $final, $subtotalTotal, $subtotalIessTotal, $subtotalTarjetaTotal, $subtotalSinTotal, $total['total'], $observacion, $id_reporte['id']);
        }
        if ($comp == "ok") {
            $msg = "ok";
        } else {
    ...
    public function generarPdf()
    {
        ...
        $productos = $this->model->getProVenta($id_usuario);
        $productosiess = $this->model->getProVentaIess($id_usuario);
        $iess = $this->model->getIess($id_usuario);
